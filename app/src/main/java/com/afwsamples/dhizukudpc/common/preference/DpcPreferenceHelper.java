...
package com.afwsamples.dhizukudpc.common.preference;
...
public class DpcPreferenceHelper {
  ...
  private int getCurrentAdmin() {
    if (Common.isDhizukuActive(mContext)) {
  ...
  private String joinRequirementList(List<String> items) {
    ...
    final StringBuilder sb = new StringBuilder(TextUtils.join(mContext.getString(R.string.requires_delimiter), items));
    if (!items.isEmpty()) {
      sb.append(mContext.getString(R.string.requires_or));
    }
    sb.append(lastItem);
    sb.append(mContext.getString(R.string.requires));
    return sb.toString();
  }
