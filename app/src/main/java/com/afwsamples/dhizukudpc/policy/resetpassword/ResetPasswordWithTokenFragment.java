...
package com.afwsamples.dhizukudpc.policy.resetpassword;
...
public class ResetPasswordWithTokenFragment extends Fragment implements View.OnClickListener {
  ...
  private void savePasswordResetTokenToPreference(byte[] token) {
    Context directBootContext = requireActivity().createDeviceProtectedStorageContext();
    SharedPreferences settings = directBootContext.getSharedPreferences(PREFS_NAME, 0);
  ...
  private void reloadTokenInfomation() {
    final byte[] token = loadPasswordResetTokenFromPreference(requireActivity());
    ...
    boolean active =
        mDpm.isResetPasswordTokenActive(DhizukuVariables.COMPONENT_NAME);
  ...
  public void onCreate(Bundle savedInstanceState) {
    ...
    mKeyguardMgr = requireActivity().getSystemService(KeyguardManager.class);
  }
  ...
  private void createNewPasswordToken() {
    ...
    if (!mDpm.setResetPasswordToken(DhizukuVariables.COMPONENT_NAME, token)) {
  ...
  private void removePasswordToken() {
    if (!mDpm.clearResetPasswordToken(DhizukuVariables.COMPONENT_NAME)) {
  ...
  private void resetPasswordWithToken() {
    ...
    if (token != null) {
      boolean result =
          mDpm.resetPasswordWithToken(DhizukuVariables.COMPONENT_NAME, password, token, flags);
      if (result) {
