...
import static com.afwsamples.dhizukudpc.common.Util.SDK_INT;
...
public class WifiEapTlsCreateDialogFragment extends DialogFragment {
  ...
  public void onCreate(Bundle savedInstanceState) {
    ...
    mWifiConfiguration = requireArguments().getParcelable(ARG_CONFIG);
  ...
  private void onSelectClientCertClicked(View view) {
    KeyChain.choosePrivateKeyAlias(requireActivity(),
        alias -> {
          ...
          requireActivity().runOnUiThread(() -> updateSelectedCert(mUserCertTextView, /* uri= */ null, alias));
        }, /* keyTypes[] */
        null, /* issuers[] */
        null, /* uri */
        null, /* alias */
  ...
  private boolean extractInputDataAndSave() {
    ...
    boolean success = WifiConfigUtil.saveWifiConfiguration(requireActivity(), mWifiConfiguration);
    if (success) {
      ...
      try {
        parseInfo =
            CertificateUtil.parsePKCS12Certificate(
                requireActivity().getContentResolver(), mUserCertUri, certPassword);
