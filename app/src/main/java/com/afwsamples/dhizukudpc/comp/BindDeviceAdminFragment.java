...
import com.afwsamples.dhizukudpc.common.preference.CustomConstraint;
...
public class BindDeviceAdminFragment extends BaseSearchablePolicyPreferenceFragment
  ...
  public boolean isAvailable(Context context) {
    DevicePolicyManager dpm = Common.binderWrapperDevicePolicyManager(requireActivity());
    return dpm.isDeviceOwnerApp(context.getPackageName()) && Util.getBindDeviceAdminTargetUsers(context).size() == 1;
  }

  @Override
  public void onCreatePreferences(Bundle savedInstanceState, String rootKey) {
    addPreferencesFromResource(R.xml.bind_device_admin_policies);
    mUserManager = (UserManager) requireActivity().getSystemService(Context.USER_SERVICE);
    ...
    mInstallCaCertificatePreference.setOnPreferenceClickListener(preference -> true);
  ...
  public boolean onPreferenceChange(Preference preference, Object newValue) {
    if (preference.getKey().equals(KEY_HIDE_PO_LAUNCHER_ICON)) {
      if (!mBindDeviceAdminServiceHelper.crossUserCall(
              service -> service.setLauncherIconHidden((boolean) newValue))) {
        Toast.makeText(requireActivity(), R.string.bind_to_profile_owner_failed, Toast.LENGTH_LONG)
                .show();
      }
      return true;
    }
